...
<script type="text/javascript">
   var replyService = (function() {
      ...
      function remove(rno, replyer, callback, errors){
    ...
    function get(rno, callback, error){
      ...
      function displayTime(timeValue){
    	...
    	// 밀리초 * 1분=60초* 1시간=60분* 하루=24
    	  ...
    	  var str = ""; //리턴할 날짜(오늘댓글이면 시간 , 어제 이전댓글이면 날짜)
...
$(document).ready(function(){
	...
	//html(ddddd) : ul태그의 안의 html 코드를 수정
	var replyUL = $(".chat");
	
	//동적으로 댓글을 만들어서 붙여주는 함수
	//요청이 성공했으면 그대 댓글을 만들어서 붙여주면 된다.
	//요청이 성공했으면 그때 댓글을 만든다 ==> callback함수에서 처리하면된다.
	showList(1);
	...
	function showList(page){
		...
		replyService.getList({
			...
			//요청 성공시 callback함수에 list를 받아온다.
			
			console.log("replyCnt : " + replyCnt);
			console.log("list : " + list);
			//만약 페이지 번호가 -1로 전달되면 마지막페이지로 가도록
			if(page == -1){
				//댓글개수가 7개일때(1페이지에 10개) 페이지개수 1개
	...
	function showReplyPage(replyCnt){
		...
		//앞페이지가 존재하는지 ??
		let prev = startNum !=1;
		// 댓글은 마지막 페이지부터 보여주니가 일단 다음페이지는 없는것으로
		let next = false;
		...
		//다음 페이지가 존재하면 (10 개 단위) next 버튼 활성화
		if(next){
			pageHtml += "<li class='page-item'>";
	...
	//앞으로 보낼 모든ajax 요청의 헤더에 csrf토큰 정보를 담아서 보낼수 있도록 설정
	...
	$("#addReplyBtn").on("click" , function(e){
		...
		//보여줄필요가 없다.
		//close버튼 이외의 다른 버튼들 다숨기고
		
		//name이 replyer인 input tage 의 값을 우리가 위에서 가져온 replyer로 설정
		modal.find("input[name='replyer']").val(replyer);
		
		modal.find("button[id != 'modalCloseBtn']").hide();
		//등록 버튼은 필요하니가 다시보이게해준다
	...
	//댓글 추가 이벤트
	//name값이 reply인 input 태그 가져오기
	var modalInputReply = modal.find("input[name='reply']");
	// name값이 replyer인 input 태그 가져오기
	var modalInputReplyer = modal.find("input[name='replyer']");
	
	
	$("#modalRegisterBtn").on("click",function(e) {
		//name 속성이 reply인 input 찾아오기 : 댓글내용
	...
	$("#modalRemoveBtn").on("click" , function(e){
		...
		//같은지 검사를 해야되므로 replyer파라미터가 필요하게 되었다.
		//그래서 파라미터를 추가해준다.
		let originalReplyer = modalInputReplyer.val();
		
		if(replyer != originalReplyer){
			alert("자신이 작성한 댓글만 삭제가 가능함.");
			modal.modal("hide");
			return;
		}
	...
	e.preventDefault(); //a태그 기본 동작 제거
	console.log("page click");
	//이동할페이지번호
	//href 속성에 페이지번호를 저장해놨으므로 꺼내서씀
