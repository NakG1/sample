...
public class BoardControllerTest {
	...
	//mvc패턴을 테스트하기 위해서 서버 없이 동작해볼수 있도록 해주는 클래스
	private MockMvc mockMvc;
	
	//테스트 메소드 실행하기 전에 항상 먼저 실행되는 메소드를 처리->@before
	//가짜 mvc 만들어 주기(기능만 테스트 할것)
	...
	public void testList() throws Exception {
		...
		.get("/board/list").param("pageNum", "2").param("amount", "10"))
		.andReturn()
		.getModelAndView()
		.getModelMap()
		.toString());
...
//		String result="";
//		result = mockMvc.perform(MockMvcRequestBuilders.post("/board/modify")//가짜 Mvc실행
//				.param("bno", "1")
